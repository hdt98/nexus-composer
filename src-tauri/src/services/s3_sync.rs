...
/// Example: `nexus-composer-sync/v2/db-v7/default/manifest.json`
...
mod tests {
    ...
    fn s3_key_uses_v2_and_correct_format() {
        ...
        assert_eq!(
            key,
            format!(
                "nexus-composer-sync/v{PROTOCOL_VERSION}/db-v{DB_COMPAT_VERSION}/default/manifest.json"
            )
        );
    }
    ...
    fn s3_key_with_custom_profile() {
        ...
        assert_eq!(
            ...
            format!("my-root/v{PROTOCOL_VERSION}/db-v{DB_COMPAT_VERSION}/work/db.sql")
        );
    }
    ...
    fn s3_key_matches_expected_pattern() {
        ...
        assert_eq!(parts[0], "nexus-composer-sync");
