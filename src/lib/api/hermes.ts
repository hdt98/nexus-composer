...
/**
 * Hermes Agent configuration API (Nexus Composer side).
 *
 * Nexus Composer intentionally keeps its Hermes surface minimal — deep configuration
 * (model, agent behavior, env vars, skills, cron, logs, analytics) lives in
 * the Hermes Web UI at http://127.0.0.1:9119. Nexus Composer only reads the `model`
