...
export interface Provider {
  ...
  // 新增：是否为商业合作伙伴
  isPartner?: boolean;
  // 可选：供应商元数据（仅存于 ~/.nexus-composer/config.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
export interface CodexCatalogModel {
  ...
  // Overrides for generated model-catalogs.json entries. Parallel-tool support
  // applies to Native Responses; input modalities apply to every profile.
...
// 存储在本地 ~/.nexus-composer/settings.json，不随数据库同步
