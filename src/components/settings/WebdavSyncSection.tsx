...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: values.remoteRoot ?? "nexus-composer-sync",
...
export function WebdavSyncSection({
  ...
  const [form, setForm] = useState(() => ({
    ...
    remoteRoot: config?.remoteRoot ?? "nexus-composer-sync",
  ...
  const [s3RemoteRoot, setS3RemoteRoot] = useState(
    s3Config?.remoteRoot ?? "nexus-composer-sync",
  );
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = config.remoteRoot ?? "nexus-composer-sync";
  ...
  useEffect(() => {
    ...
    setS3RemoteRoot(s3Config.remoteRoot ?? "nexus-composer-sync");
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: form.remoteRoot.trim() || "nexus-composer-sync",
  ...
  const buildS3Settings = useCallback((): S3SyncSettings => {
    return {
      ...
      remoteRoot: s3RemoteRoot.trim() || "nexus-composer-sync",
  ...
  const currentRemotePath = `/${form.remoteRoot.trim() || "nexus-composer-sync"}/v2/db-v6/${form.profile.trim() || "default"}`;
  const currentS3RemotePath = `${s3Bucket.trim() || "bucket"}/${s3RemoteRoot.trim() || "nexus-composer-sync"}/v2/db-v6/${s3Profile.trim() || "default"}`;
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4">
              ...
              <Input
                ...
                placeholder="nexus-composer-sync"
                className="text-xs flex-1"
                disabled={isLoading}
              />
            </div>
